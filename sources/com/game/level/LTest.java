...
public class LTest extends Level {
	...
	public LTest() {
		...
		// //add(new TileTest(str, (double) j*blockSize, (double) i*blockSize,
	...
	public int[][] generate(int[][] array, int startX, int startY, int rooms, float split) {
		...
		while (steps > 0) {
			...
			// int xoff = x + ioff;
			// int yoff = y + joff;
			// if (xoff > -1 && xoff < width && yoff > -1 && yoff < height && (xoff == x ^
			// yoff == y)
			...
			/*
			 * Point neighbor = neighbors.get(Random.nextInt(neighbors.size())); x =
			 * neighbor.x; y = neighbor.y;
			 * 
			 * int b = (y + 1 < height && array[x][y + 1] != EMPTY) ? 1 : 0; int g = (x - 1
			 * > -1 && array[x - 1][y] != EMPTY) ? 1 : 0; int h = (y - 1 > -1 && x < height
			 * && array[x][y - 1] != EMPTY) ? 1 : 0; int d = (x + 1 < width && array[x +
			 * 1][y] != EMPTY) ? 1 : 0; array[x][y] = b << 3 | h << 2 | d << 1 | g;
			 * 
			 * neighbors.remove(neighbor); if (neighbors.size() != 0 && Random.nextDouble()
			 * >= split) { neighbor = neighbors.get(Random.nextInt(neighbors.size())); array
			 * = generate(array, neighbor.x, neighbor.y, steps - 1, split/4); //split }
			 */
			steps--;
			// split += .05f;
		}
